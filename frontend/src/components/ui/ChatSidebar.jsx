...
export default function ChatSidebar() {
  const {
    ...
    selectedText, pendingMessage, clearPendingMessage,
    histories, addMessage, updateLastMessage,
    setTyping, isTyping, clearHistory, setActiveTopic,
  } = useChatStore();
  ...
  return (
    <>
      {isOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-20 z-40 no-print"
             onClick={closeChat} />
      )}
      ...
      <div className={`fixed top-0 right-0 h-full w-96 bg-white shadow-2xl z-50
        ...
        {/* Header */}
        <div className="bg-brand-orange px-4 py-3 flex items-center justify-between flex-shrink-0">
          ...
          <button onClick={closeChat}
                  className="text-white hover:text-orange-200 text-2xl font-bold">×</button>
        </div>
        ...
        {notes.length > 1 && (
          <div className="px-3 py-2 border-b border-gray-100 flex-shrink-0">
            ...
            >
              {notes.map((n) => (
                <option key={n.topic} value={n.topic}>{n.topic}</option>
              ))}
            </select>
          </div>
        )}
        ...
        {selectedText && (
          <div className="mx-3 mt-2 flex-shrink-0 bg-orange-50 border border-orange-200
            ...
            <p className="text-xs text-orange-600 font-bold mb-0.5">📌 Selected text:</p>
        ...
        <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3">
          {messages.length === 0 && (
            <div className="text-center py-8">
              ...
              <p className="text-sm font-bold text-gray-700 mb-1">Ask me anything about</p>
              <p className="text-sm text-brand-orange font-black">{activeTopic}</p>
              <p className="text-xs text-gray-400 mt-2">
                I know your notes — ask me to simplify, give examples, or help with exam answers.
              </p>
        ...
        {messages.length > 0 && (
          <div className="px-3 flex-shrink-0">
            <button onClick={() => clearHistory(activeTopic)}
